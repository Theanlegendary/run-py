# -*- coding: utf-8 -*-
"""
update_august_tracking_file.py
Generates the complete August Delivered Tracking Status Logs with:
  1. Filter for all bills DONE DELIVERED in August (51,555 bills)
  2. Added VAS column (e.g. VTT, VBP, NTN)
  3. Added LOG 430 | UNIT 430 | TIME 430 and all other post-402 status codes in order
"""
...
def run_update(detail_xlsx_path, out_path, max_workers=60):
    with open(CONFIG_PATH, "r", encoding="utf-8") as f:
        cfg = json.load(f)

    test_ids = load_test_ids()
    print(f"[INFO] Loading {detail_xlsx_path}...")
    df = pd.read_excel(detail_xlsx_path)
    df.columns = [str(c).strip().upper() for c in df.columns]

    st_col = next((c for c in df.columns if "CURRENT STATUS" in c or "STATUS" in c), None)
    if st_col:
        df_del = df[df[st_col].astype(str).str.contains("410", na=False)].copy()
    else:
        df_del = df.copy()

    print(f"[INFO] Total rows: {len(df)} | Delivered in August: {len(df_del)}")
    ...
    print(f"[INFO] Fetching tracking trips & VAS for {len(unique_oids)} bills with {max_workers} threads...")
    results = []
    ...
    print(f"[INFO] Final active status codes ({len(all_final_codes)}): {all_final_codes}")

    print(f"[INFO] Writing to Excel: {out_path}...")
    wb = Workbook()
    ws = wb.active
    ...
    wb.save(out_path)
    print(f"[SUCCESS] Saved report to: {out_path}")
    return out_path

if __name__ == "__main__":
    src = os.path.join(HERE, "latest_august_detail.xlsx")
    out_xlsx = rf"C:\Users\DELL\Desktop\Bill_Tracking_Status_Logs_Done_Delivered_01Aug_31Aug_20260904.xlsx"
    run_update(src, out_xlsx)
