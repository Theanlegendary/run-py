# -*- coding: utf-8 -*-
"""
generate_full_template_tracking_report.py
Full Standard Template for Tracking Status Logs (All Status Codes 110-600) with:
...
def run_full_template_export(detail_xlsx_path, out_paths, max_workers=70):
    ...
    print(f"[INFO] Building Excel with Full Standard Template ({len(FULL_STATUS_CODES_TEMPLATE)} status codes)...")
    wb = Workbook()
    ws = wb.active
    ...
    t_cell = ws.cell(1, 1, f"DELIVERED BILL TRACKING STATUS LOGS REPORT — AUGUST 2026 (Full Standard Template: 110–600 | {len(results)} Bills)")
    t_cell.font = f_title
...
if __name__ == "__main__":
    ...
    run_full_template_export(src, paths)
